        ...
        async function fetchProjects() {
            try {
                ...
                const readmeresponse = await fetch(`https://api.github.com/users/${GITHUB_USERNAME}/repos/readme`)
...
const isFlutter = repo.language === 'Dart'
  || (repo.topics && (repo.topics.includes('flutter') || repo.topics.includes('dart')))
  ...
  || repo.name.toLowerCase().includes('dart');
        ...
        function createProjectCard(repo, index) {
            ...
            const hue = repo.name.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0) % 360;
            const isFlutter = repo.language === 'Dart';
        ...
        function openModal(repo, hue) {
            ...
            const createdDate = new Date(repo.created_at).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
            const updatedDate = new Date(repo.updated_at).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
